	...
	if((strcmp(t1.lexeme_name,"int")==0)||(strcmp(t1.lexeme_name,"char")==0))
	{
		...
		else
		{
			if(threshold<3)
			{
				...
				printf("\n\nYou were missing a ';' here!!It has undergone panic mode recovery\n");
				fsetpos(fp,&pos);
				proc_dec(fp);
			...
			{
			printf("\n\nCompilation Error!!\nYou are missing a ';' at line number %d .\nThreshold for panic mode recovery exceeded\n",t2.rowno);
			exit(0);
			}
		}
...
void proc_id_list(FILE *fp)
{
	...
	else
	{
		if(threshold<3)
		{
			...
			//printf("%s",t1.lexeme_name);
			printf("\n\nYou were missing an identifier at line number %d .\nIt has undergone panic mode recovery\n",t2.rowno); //exit(0);
			fsetpos(fp,&pos);
		}

		else
		{
			printf("\n\nCompilation Error!!\nYou are missing an identifier .\nThreshold limit exceeded!!No more recovery\n");
			exit(0);
		}
	}
